...
class WeatherModel{
  ...
  Future<dynamic> getLocationWeather() async {
    ...
    var weatherData = await getData( '$kApiMapUrl?lat=${latitude}&lon=${longitude}&cnt=$cnt&appid=$kApiKey&units=metric');
    print("returning weather data");
    return weatherData;
  }
   Future<dynamic> getCurrentWeather() async {
     await getCurrentLocation();
     var currentWeather = await getData('https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=$kApiKey&units=metric');
